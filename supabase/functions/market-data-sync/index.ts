...
async function syncMarketData(supabase: any, apiKey: string) {
  ...
  try {
    ...
    console.log(
      `Found ${commodities?.length} commodities and ${markets?.length} markets`
    );
    ...
    for (const commodity of commodities || []) {
      ...
      const apiCropName =
        cropMapping[commodity.name.toLowerCase()] ||
      ...
      for (const source of marketDataSources) {
        try {
          ...
          if (response.ok) {
            ...
            console.log(
              `Got ${records.length} records from ${source.name} for ${commodity.name}`
            );

            // Process each record
            for (const record of records.slice(0, 10)) {
              // Limit to 10 records per commodity
              try {
                ...
                const market = markets?.find(
                  (m) =>
                    m.district
                      .toLowerCase()
                      .includes(record.district?.toLowerCase() || "") ||
                    record.market
                      ?.toLowerCase()
                      .includes(m.district.toLowerCase())
                );
                ...
                const maxPrice = parseFloat(
                  record.max_price || record.min_price || "0"
                );
                ...
                if (existingPrice) {
                  ...
                  priceData.trend =
                    change > 50 ? "up" : change < -50 ? "down" : "stable";
                ...
                console.error(
                  `Error processing record for ${commodity.name}:`,
                  recordError
                );
              }
            }
...
async function generateMarketInsights(supabase: any) {
  try {
    ...
    if (!openRouterKey) {
      console.warn(
        "OPENROUTER_API_KEY not configured; skipping AI market insights"
      );
      return;
    }
    ...
    for (const [commodity, prices] of Object.entries(commodityGroups)) {
      try {
        ...
        const avgPrice =
          priceArray.reduce((sum: number, p: any) => sum + p.modal_price, 0) /
          priceArray.length;
        const trend = priceArray[0]?.trend || "stable";
        const region =
          priceArray[0]?.market_locations?.district || "Tamil Nadu";

        // Generate AI insight using free model
        const prompt = `Analyze market trend for ${commodity} in ${region}. Average price: ₹${avgPrice.toFixed(
          2
        )}, Current trend: ${trend}. Provide brief insight for Tamil Nadu farmers in 1-2 sentences.`;
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You are an agricultural market analyst. Provide concise, practical insights for farmers.",
                },
                {
                  role: "user",
                  content: prompt,
        ...
        console.error(
          `Error generating insight for ${commodity}:`,
          insightError
        );
      }
    }
    ...
    const sampleNews = [
      {
        ...
        content:
          "Rice prices have increased by 5-8% across major markets in Tamil Nadu due to reduced arrivals and increased demand.",
      ...
      },
      {
        ...
        content:
          "Farmers are advised to monitor weather conditions as rainfall patterns may affect upcoming harvest and market prices.",
        category: "weather",
        region: "Tamil Nadu",
